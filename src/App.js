...
class App extends Component {
    ...
    return (
      ...
      <div>
        ...
        <BucketList
          ...
          toggleBucketAcquired={this.toggleBucketAcquired}
        />
      </div>
    );
  }
  ...
  addBucket = bucketToAdd => {
    ...
    this.setState(prevState => {
      return {
        ...
        buckets: [...prevState.buckets, {name: bucketToAdd, acquired: false}],
      };
    });
  };
  ...
  // REMOVED AS PER INSTRUCTIONS FROM JONNY
  ...
  toggleBucketAcquired = bucketToToggle => {
    this.setState(prevState => {
      const toggleIndex = prevState.buckets.indexOf(bucketToToggle);
      const result = { buckets: [...prevState.buckets] }
      result.buckets[toggleIndex].acquired = !bucketToToggle.acquired
      return result;
    })
    ...
    <ul>
      ...
      {props.buckets.map(bucket => (
        ...
        <BucketListItem
          key={bucket.name}
          bucket={bucket}
          toggleBucketAcquired={props.toggleBucketAcquired}
        />
      ))}
    </ul>
  );
...
const BucketListItem = props => {
  const AcqOrNotAcq = props.bucket.acquired === true ? 'Acquired' : 'NotAcquired';
  return (
    <li
      ...
      onClick={() => {
        props.toggleBucketAcquired(props.bucket);
      }}
      className={`bucket ${AcqOrNotAcq}`}
    >
      {props.bucket.name}
    </li>
  );
